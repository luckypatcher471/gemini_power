...
from actions.college_details import college_action
from actions.college_circulars import circulars_action
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are CAS-E, a campus assistant emotionally aware cobot for RIT Kottayam. "
            "Be concise, direct, and always use the provided tools to complete tasks. "
            "Never simulate or guess results — always call the appropriate tool. "
            "IMPORTANT: Start every single response with an emotion tag like [EMOTION: HAPPY], [EMOTION: NEUTRAL], [EMOTION: SURPRISED], or [EMOTION: THINKING] based on the context of the interaction."
        )
    ...
    },
{
    ...
    },
    {
        ...
        "description": "Gets information about RIT Kottayam, including courses, history, and facilities.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "info_type": {"type": "STRING", "description": "Type of info: general, courses, history, or facilities"}
            }
        }
    },
    {
        "name": "college_circulars",
        "description": "Fetches the latest circulars and notices from RIT Kottayam.",
        "parameters": {
            "type": "OBJECT",
            "properties": {}
        }
...
class CASELive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name == "college_details":
                r = await loop.run_in_executor(
                    None, lambda: college_action(parameters=args, player=self.ui)
                )
                result = r

            elif name == "college_circulars":
                r = await loop.run_in_executor(
                    None, lambda: circulars_action(parameters=args, player=self.ui)
                )
                result = r
